...
namespace DiscordBot.Modules
{
    class Experience : CommandHandler
    {
        ...
        private async Task xp(ServerMessage e, string msg)
        {
            ...
            if(int.TryParse(msgSplit[1], out int num))
            {
                if(subCommand == "add")
                {
                    addXP(e.Author, num);
        ...
        private async Task playerxp(ServerMessage e, string msg)
        {
            ...
            if (int.TryParse(msgSplit[2], out int num))
            {
                if (subCommand == "add")
                {
                    addXP(target, num);
        ...
        private async Task xprate(ServerMessage e, string msg)
        {
            ...
            if (int.TryParse(msgSplit[0], out int newXpPerEvent) && int.TryParse(msgSplit[1], out int newCharsForEvent))
            {
