...
namespace DiscordBot.Modules
{
    ...
    public class Proposal
    {
        ...
        async public Task<bool> start(Action<Proposal> store, VirtualServer serv, ServerMessage msg, ulong propId)
        {
            ...
            TimeOfCompletion = DateTime.Now + TotalDuration;
            store(this);
        ...
        public string PropPrint(bool isArchived, ulong id)
        {
            ...
            if(isArchived)
            {
                msg = "=== ARCHIVED PROPOSAL #" + id + " - '" + Name + "' ===";
            }
            else
            {
                msg = "=== PROPOSAL #" + id + " - '" + Name + "' ===";
            }
            endLength = msg.Length;
            msg += "\n\n Duration: " + TotalDuration;
            if (Started)
            {
                msg += "\n Started: " + TimeOfCompletion.Subtract(TotalDuration);
                if (isArchived)
                {
                    msg += "\n Ended: " + TimeOfCompletion;
                    msg += "\n Reason: " + reasonForEnding;
                }
                else
                {
                    msg += "\n Time Remaining: " + TimeOfCompletion.Subtract(DateTime.Now);
                }
            }
            msg += "\n\n" + Text;
//            if (this.Started)
//            {
                ...
                if (Options.Count > 0)
                {
                    List<Tuple<int, string>> optsList = new List<Tuple<int, string>>(Options.Count);
...
*/                else if (split[0].Equals("Abort", StringComparison.InvariantCultureIgnoreCase))
        ...
        private void queuePropEnd(Proposal prop, ulong propId)
        {
            ...
            if (prop.TimeOfCompletion != default)
        ...
        private async Task proposalComplete(Proposal prop, ulong propId)
        {
            if (Proposals.Remove(propId))
            {
                ...
                pastProposals.Add(propId, new LazyDiscLoaded<Proposal>(prop));
