...
namespace DiscordBot.Modules
{
    ...
    class Proxy : CommandHandler
    {
        ...
        {
            IChannel settingsChannel = e.Channel;
            ...
            else
            {
                ...
                if (triggerCommands)
                {
                    ...
                    var eClone = new ServerMessage(e.Server, e.isDM, true, e.Channel, newMsg as IUserMessage, e.Author);
                    await server.messageRecieved(eClone);
                }
            }
            try
            {
                var delReq = new RequestOptions();
                delReq.RetryMode = RetryMode.AlwaysRetry;
        ...
        private async Task deleteLastMsg(ServerMessage e, string msg)
        {
            ...
            if(idx != -1)
            {
                ...
                {
                    msgs.AddRange(msgList.Where(x => x.Source == MessageSource.Webhook));
                }
